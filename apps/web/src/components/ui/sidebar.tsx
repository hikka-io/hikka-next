...
>(({ className, asChild = false, ...props }, ref) => {
    ...
    return (
        <Comp
            ...
            className={cn(
                'flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-hidden ring-sidebar-ring transition-[margin,opa] duration-150 ease-in-out focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const SidebarShadowTrigger = React.forwardRef<
    HTMLDivElement,
    React.ComponentProps<'div'>
>(({ className, ...props }, ref) => {
    const { toggleSidebar } = useSidebar();
...
>(({ className, ...props }, ref) => {
    ...
    return (
        <div
            onClick={toggleSidebar}
            className={cn(
                'fixed inset-0 z-50 h-dvh bg-black/30 backdrop-blur-xs group-data-[state=collapsed]:hidden group-data-[state=open]:animate-in group-data-[state=collapsed]:animate-out group-data-[state=collapsed]:fade-out-0 group-data-[state=open]:fade-in-0',
                className,
            )}
            {...props}
            ref={ref}
